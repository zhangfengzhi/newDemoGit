    ...
    function findNotice(data,init){
    	isOnLineStatus(function(is_online, line_type) {
			if (is_online) {
				...
				api.ajax({
					    url: path_url+'/notice/getNoticeList?page_number='+data.page_number+'&page_size='+data.page_size+'&register_id='+gzs_id+'&random='+Math.random(),
					    method: 'post',
					    timeout: 30,
					    cache:false,
					    dataTpye:'json',	
					    ...
					    if (ret) {
					       ...
					       if($api.html(kcDiv).indexOf("暂无通知")>-1){
			       		   	    $api.html(kcDiv, "");  
			       	   	   }
					        for(var i=0;i<txt.list.length;i++){
					        var index=Math.ceil(Math.random()*9);
					        var contents=txt.list[i].content;
					        if(contents.length>30){
					        contents=contents.substr(0,30)+"...";
					        }
					         var html='<a href="javascript:void(0);" class="item Fix hightitem" tapmode onclick="openNotice('+txt.list[i].notice_id+',\''+txt.list[i].title+'\');">'
					            		+'<div class="cnt">'
								            	...
								            	+'<div class="wrap2">'	
									            	+'<div class="content">'	
									                	...
									                	+'<div class="comment">'
									                    	+'<span>'+contents+'</span>'	
									                	+'</div>'	
									                	+'<span class="classify" id="ljsj'+txt.list[i].notice_id+'">发布时间 ：'+	txt.list[i].create_time
									                	+'</span>'	
									            	+'</div>'	
								            	+'</div>'           	
							           		+'</div>'	
					        ...
					        if(txt.list.length==0){//说明没查出，需要判断原有div里边是否有，如果有，那么什么都不做，如果没有，加上没有培训班的提示。
						  		var n=$api.html(kcDiv);
						   		if(n==null||n==""){			      
						      		$api.append(kcDiv,"<div align='center' style='margin-top:50px;'><h3>暂无通知</h3></div>");
						  		}else{
						  			api.toast({
									    msg: '已加载所有数据',
									    duration:2000,
	...
	function openNotice(notice_id,notice_title){
		...
		api.openWin ({
		name: 'show_notice_window',
		url: './show_notice_window.html',
		bounces : false,
				opaque : false,
				showProgress : false,
    ...
    apiready = function(){
       ...
       api.ajax({
					    url: path_url+'/space/getNewsRegisterId?a_id='+gzs_id+'&a_identity_id=107&random='+Math.random(),
					    method: 'post',
					    timeout: 30,
					    cache:false,
					    dataTpye:'json',	
