...
//                      openChangeFrame();
//                  }else{
                        popAlertAndShowTitle('提示', Base64.encode('当前还没有教材信息，请先到“我”，找到设置教材功能，设置教材！'), 'api.execScript({name : "kejianJs_index_window",script : "openSzJcFrame(1);"});');
//                  }
...
'Cookie' : 'person_id=' + $api.getStorage("person_id") + '; identity_id=' + $api.getStorage("identity") + '; token=' + $api.getStorage("token_ypt") + ';'
			...
			api.hideProgress();
			if(err){
				popToast('获取课件列表失败，请稍候重试');
			}else{
				if(ret.success){
					for(var i = 0; i < ret.list.length; i++){
						ret.list[i].slot_time = GetDateDiff(ret.list[i].create_time);
						ret.list[i].name_base64 = Base64.encode(ret.list[i].resource_title);
						ret.list[i].resource_title = ret.list[i].resource_title.replaceAll(' ','&nbsp;');
						if (BASE_APP_TYPE == 1) {
							ret.list[i].thumb_id = url_path + BASE_THUMB_BEGIN + ret.list[i].thumb_id.substring(0, 2) + '\/' + ret.list[i].thumb_id + BASE_THUMB_END;
						} else {
							ret.list[i].thumb_id = BASE_URL_ACTION + BASE_THUMB_BEGIN + ret.list[i].thumb_id.substring(0, 2) + '\/' + ret.list[i].thumb_id + BASE_THUMB_END;			
						}
					}
...
/*
*author:zhaoj
*function:获取精品课件案列表数据
*date：20170401
*/
function getBoutiqueLeadList(){
	api.ajax({
		url : BASE_URL_ACTION + '/resCtl/getJPResourceList?random_num='+creatRandomNum()+'&keyword=&pageNumber=1&pageSize=10&nid='+lobal_variable.structure_id+'&schoolId='+$api.getStorage("school_id") +'&schemeId='+lobal_variable.scheme_id+'&appTypeId=3&cnode=0&isRoot=0&groupId=1',
		method : 'get',
		timeout : 0,
		dataType : 'json',
		headers : {
...
*date：20160523
*/
function popConfirm(id,resource_id_int,res_id,type,msg){
	api.confirm({
		title : "提示",
		msg : msg,
		buttons : ["确定","取消"]
		...
		if(1 == ret.buttonIndex) {
			if(type == 0){
				deletePublishByPid(res_id);
			}else if(type == 1){
				deleteResourceByIds(res_id,resource_id_int,id);
			}
		}
	});
}
...
*date：20170401
*/
function deletePublishByPid(res_id){
...
*date：20170401
*/
function deleteResourceByIds(res_id,resource_id_int,id){
...
*date：20170401
*/
function insertResourceBaseInfo(){
	showSelfProgress('保存中...'); 
	$api.css($api.byId('j_shadow'),'z-index:3');
	var value = $api.trim($api.val($api.byId('fl_val')));
	if(value.length == 0){
		popAlert('课件名称不可以为空');
	...
	api.ajax({
		...
		data : {
			values : {
				...
				"beike_type" : 0,
				"bk_type_name" : -1,
				"file_id" : api.pageParam.file_id,
				"group_id" : 2,
				"identity_id" : $api.getStorage("identity"),
				"person_id" : $api.getStorage("person_id"),
				"person_name" : Base64.encode($api.getStorage("person_name")),
				"res_type" : 1,
				"resource_format" : api.pageParam.resource_format,
				"m3u8_status" : 0,
				"structure_id" : lobal_variable.structure_id,
				"resource_title" : Base64.encode(value),
				"is_check": 0,
			}
		},
		headers : {
'Cookie' : 'person_id=' + $api.getStorage("person_id") + '; identity_id=' + $api.getStorage("identity") + '; token=' + $api.getStorage("token_ypt") + ';'
			...
			if(err){
				...
				if(ret.success){
					saveResource(0,ret.resource_myinfo_id,'');
				}else{
					popToast('选用失败');
					api.hideProgress();
					$api.css($api.byId('j_shadow'),'z-index:-1');
				...
				if(err){
					...
					if(ret.success){
						if(type){
							...
							popToast('选用成功');
//							if(api.uiMode == 'pad'){
//								commonExecScript('kejianIpad_index_window','kejianIpad_index_frame','updataList();',1);//切换index列表数据
//								setTimeout(function(){
//									commonExecScript('kejianIpad_index_window','','back()',0);
//								},2000);
//							}else{
								commonExecScript('kejianJs_index_window','','changeBtnTitle(0)',0);
								setTimeout(function(){
			...
			if(ret){
				if(ret.success){
					setTimeout(function(){
						popToast('添加成功');
						api.execScript({
							name : 'kejianJs_stu_group_window',
							frameName : 'kejianJs_stu_group_frame',
							script : 'initData();'
						});
					},1500);
					setTimeout(function(){
						api.hideProgress();
						commonCloseFrame("kejianJs_group_name_frame");
					},1600);
				}else{
					popTipInfo(1);
				}
