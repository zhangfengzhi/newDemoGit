...
function getSubjectByStudentId(callback){
	commonGetStudentInfoByParentId(total_param.person_id, total_param.identity, function(is_true, stu_info_json){
		if(is_true) {
			if(BASE_CRM_TYPE){
				api.ajax({
				     url:BASE_URL_ACTION + '/yxx/gameOrTopic/resCtl/getstussinfo?type='+2+'&random_num='+ creatRandomNum() +　'&person_id='+stu_info_json.student_id+'&identity_id=6',
				    method : 'get',
				    dataType : 'json', 
					cache: false,
					timeout : 30,
			    		...
			    		if(ret.success){
			    			getmoudelsubject(stu_info_json.student_id,function(mode_ret){
				    			...
				    			for(var i = 0; i < ret.list.length; i++){
				    				for(var j = 0; j < mode_ret.zt_subject_ids.length; j++){
				    					if(mode_ret.zt_subject_ids[j].topic_subject == ret.list[i].subject_id){
...
function getGameOrTopicList(callback){
	...
	api.ajax({
		url : BASE_URL_ACTION + '/yxx/gameOrTopic/resCtl/getGameOrTopicList?page_size='+page_size+'&page_number='+currentPage+"&random_num="+creatRandomNum()+ "&gameOrTopic=2"+'&keyword='+keyword+'&type_id='+type_id+'&subject_id='+subject_id+'&order_type='+order_type,
		method : 'get',
		dataType : 'json',
		cache : false,
		timeout:30,
...
function getGameTopicPreview(callback){
	commonGetStudentInfoByParentId(total_param.person_id, total_param.identity, function(is_true, stu_info_json){
		if(is_true) {
			api.ajax({
				url :BASE_URL_ACTION + '/yxx/gameOrTopic/resCtl/getGameTopicPreview?student_id='+stu_info_json.student_id+'&class_id='+$api.getStorage("class_id")+'&game_or_topic=2&random_num='+creatRandomNum(),
				method : 'get',
				dataType : 'json',
				cache : false,
				timeout:30,
