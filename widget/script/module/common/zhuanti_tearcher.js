...
function getSubjectByStudentId(callback){
	if(BASE_CRM_TYPE){
		...
		api.ajax({
			url : BASE_URL_ACTION + '/crm/getCrmInfoByPersonId',
			method : 'post',
			dataType : 'json',
			cache : false,
			timeout:30,
			...
			if(err){
				...
				if(ret.success) {
					...
					if(product_list.length>0){
						for(var i=0;i<product_list.length;i++){
							if(product_list[i].product_id=='lxkt'){
								isHaveZt = 1;
								var data = {};
								data.ssvt_list = product_list[i].ssvt_list;
								callback(data);
...
function getGameOrTopicList(callback){
	api.ajax({
		url : BASE_URL_ACTION + '/yxx/gameOrTopic/resCtl/getGameOrTopicList?page_size='+BASE_PAGE_SIZE+'&page_number='+currentPage+"&random_num="+creatRandomNum()+ "&gameOrTopic=2"+'&keyword='+keyword+'&type_id='+type_id+'&subject_id='+subject_id+'&order_type='+order_type,
		method : 'get',
		dataType : 'json',
		cache : false,
		timeout:30,
