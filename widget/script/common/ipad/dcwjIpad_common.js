...
function openMyNavigationBar(type){
	...
	if(type == 1){
		...
		winName:'dcwjIpad_index_window',
		frameName:'dcwjIpad_index_frame'
	};
...
function haveOpt(type,answer_id,id,name_base64){
	if(type == 0){
		...
		var functionArray = ['commonOpenWin("dcwjIpad_detail_window", "dcwjIpad_detail_window.html", false, false, {"header_h":'+header_h+',"id":'+id+',"type":1,"name_base64":"'+name_base64+'"})','viewStatistics(1,'+id+',"'+name_base64+'")'];
	}else if(type == 1){
		var buttonArray = ['查看答题','查看结果'];
		var functionArray = ['commonOpenWin("dcwjIpad_detail_window", "dcwjIpad_detail_window.html", false, false, {"header_h":'+header_h+',"id":'+id+',"answer_id":'+answer_id+',"type":2,"name_base64":"'+name_base64+'"})','viewStatistics(0,'+id+',"'+name_base64+'")'];
	}else{
		var buttonArray = ['查看答题','查看结果'];
		var functionArray = ['commonOpenWin("dcwjIpad_detail_window", "dcwjIpad_detail_window.html", false, false, {"header_h":'+header_h+',"id":'+id+',"answer_id":'+answer_id+',"type":2,"name_base64":"'+name_base64+'"})','popAlert("此次调查问卷的统计保密，不支持查看！")'];
	}
	api.actionSheet({
	    cancelTitle: '取消',
	    buttons: buttonArray,
...
function viewStatistics(type,id,name_base64){
	...
	commonOpenWin('dcwjIpad_tj_detail_window', 'dcwjIpad_tj_detail_window.html', false, false, pageParamJson);
}
/*
*作者:zhaoj
*功能:删除我创建的
