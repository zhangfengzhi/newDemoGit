...
function openNavigationBarToHtml(data) {
	var params = {
		...
		winName:'zuoye_index_window',
		frameName:'zuoye_index_frame'
	};
...
*日期：20160912
*/
function openTjNavigationBarToHtml(data,name) {
	var params = {
		y : header_h-1,
		itemSize : {
			w : data.width
...
*日期：20160912
*/
function openPublicOpt(zy_id,zy_name_base64,zy_desc_base64,subject_id){
	...
	api.actionSheet({
	    ...
	    if( ret ){
	         if(ret.buttonIndex == 1){
	         	//查看检测
	         	openCommonWin('zuoye_detail_window',zy_name_base64,zy_id,'',1,1);
	         }else if(ret.buttonIndex == 2){
	         	//统计检测
	         	openCommonWin('zuoye_tj_window',zy_name_base64,zy_id,'','','');
	         }else if(ret.buttonIndex == 3){
	         	//取消发布
		        popConfirm(zy_id,3,"确定取消检测“"+Base64.decode(zy_name_base64).substring(0,4)+"...”的发布吗？")
	         }else if(ret.buttonIndex == 4){
	         	//共享检测
	         	openCommonWin('zuoye_share_window',zy_name_base64,zy_id,1,'','');
...
function openYunListOpt(zy_id,zy_name_base64,zy_desc_base64,subject_id){
	api.actionSheet({
	    ...
	    if( ret ){
	         if(ret.buttonIndex == 1){
	         	//查看检测
	         	openCommonWin('zuoye_detail_window',zy_name_base64,zy_id,'',0,0);
	         }else if(ret.buttonIndex == 2){
	         	//发布检测
...
*日期：20160912
*/
function openNoPublicOpt(zy_id,zy_name_base64,zy_desc_base64,subject_id){
	isShowFooterBtn(0);//不显示下方按钮
	api.actionSheet({
	    cancelTitle: '取消',
	    ...
	    if( ret ){
	         if(ret.buttonIndex == 1){
	         	//查看检测
	         	openCommonWin('zuoye_detail_window',zy_name_base64,zy_id,'',0,1);
	         }else if(ret.buttonIndex == 2){
	         	//发布检测
	         	openPublishWin(zy_id,zy_name_base64,subject_id);
	         }else if(ret.buttonIndex == 3){
...
*日期：20160912
*/
function openCommonWin(name,zy_name_base64,zy_id,o_type,isPublic,isXiafa){
	api.openWin({
		name : name,
		url : name+'.html',
		pageParam : {
...
function openStListWindow(student_id,head_img,student_name){
	api.openWin({
		name : 'zuoye_st_list_window',
		url : 'zuoye_st_list_window.html',
		pageParam : {
			header_h : header_h,
...
*date：20160330
*/
function openPublishWin(zy_id,zy_name_base64,subject_id){
	api.openWin({
		name : 'zuoye_publish_window',
		url : 'zuoye_publish_window.html',
		pageParam : {
			header_h : header_h,
			zy_id : zy_id,
			zy_name_base64 : zy_name_base64,
...
*作者:zhaoj
*功能:打开学生单一试题的详细页面
*参数:type=1,就是从试题统计页面进入的，显示学生名单，不然就只显示题目
*日期：20160918
*/
function showQueDetail(question_id_char,sort,qt_name,qt_type,type,wd_totalRow,zc_totalRow,mf_totalRow,qt_totalRow,obj_type,obj_id, zy_id){
	api.execScript({
		name : 'zuoye_tj_window',
	...
	api.openFrame({
		name : 'zuoye_single_frame',
		scrollToTop : true,
		allowEdit : false,
		url : 'zuoye_single_frame.html',
		rect : {
			x : 0,
			y : header_h,
			w : 'auto',
...
function openZtPersonListFrame(zy_id,obj_type,obj_id,yingj_totalRow,yij_totalRow,wj_totalRow){
	...
	api.openFrame({
		name : 'zuoye_person_frame',
		scrollToTop : true,
		allowEdit : false,
		url : 'zuoye_person_frame.html',
		rect : {
			x : 0,
			y : header_h,
			w : 'auto',
...
function saveZuoye(zy_name_base64,zy_id){
	api.execScript({
		name : 'zuoye_index_window',
		script : 'reBackType("save");'
	});
	api.openFrame({
			name : 'zuoye_save_frame',
			scrollToTop : true,
			allowEdit : true,
			url : 'zuoye_save_frame.html',
...
function openWriteWin(type,sort){
	api.openWin({
		name : 'zuoye_st_window',
		url : 'zuoye_st_window.html',
		pageParam : {
			header_h : header_h,
			zy_id : zy_id,
			zy_name_base64 : api.pageParam.zy_name_base64,
...
function isShowFooterBtn(type){
	api.execScript({
		name : 'zuoye_index_window',
		script : 'isShowWinFooterBtn('+type+');'
	});
	if(type){
		var height = header_h+39+55;
	}else{
		var height = header_h+39;
	}
	api.setFrameAttr({
	    name: 'zuoye_index_frame',
	    rect: {
	        x: 0,
	        y: header_h+39,
	        w: 'auto',
			...
			if(ret){
				if(ret.success){
					setTimeout(function(){
						popToast('添加成功');
						api.execScript({
							name : 'zuoye_stu_group_window',
							frameName : 'zuoye_stu_group_frame',
							script : 'initData();'
						});
					},1500);
					setTimeout(function(){
						api.hideProgress();
						commonCloseFrame("zuoye_group_name_frame");
					},1600);
				}else{
					popTipInfo(1);
				}
